...
const remoteAuthDb = secrets.gce.authDatabase;
...
const uri = 'mongodb://' +
            ...
            remotePassword + '@' +
            remoteHost + ':' +
            remotePort + '/?authMechanism=MONGODB-CR&authSource=' +
            remoteAuthDb;
...
const guardedProcessFiling = guard(condition, function processFiling(data, dbClient) {
...
MongoClient.connect(uri, (e, dbClient) => {
  if (e) {
    throw e;
  }
  s3Stream
    ...
    .pipe(eventStream.mapSync(function(data) {
      ...
      guardedProcessFiling(data, dbClient);
      return;
    }))
    .on('error', function(err) {
